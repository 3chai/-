...
def generate_timesheet(file_bytes, preset):
    ...
    valid_cells = [c for c in df_raw.columns if c in cell_offsets]
        ...
        for _, row in df_page.iterrows():
            ...
            for book_col, position in book_positions.items():
                ...
                if position.startswith("before_"):
                    ...
                    x_base = cell_x_positions_true.get(target, 0) - 20 * scale_factor_w
                elif position.startswith("between_"):
                    left, right = position.replace("between_", "").split("_")
                    x_base = (cell_x_positions_true.get(left, 0) + cell_x_positions_true.get(right, 0)) / 2
                else:  # after_
                    left = position.replace("after_", "")
                    x_base = cell_x_positions_true.get(left, 0) + 20 * scale_factor_w
                x_draw = x_base if column == 0 else x_base + column_offset_x
                draw.text((x_draw, y_true), book_val, fill=(0, 0, 0, 255), font=font_large_scaled)
...
# UI
st.title("ちゃいむしーと Web版 v1.9.2 ＋ book列対応")
